...
organizationsRouter.post("/", requireAuth, requireRole("leader"), async (req, res) => {
...
organizationsRouter.put("/:id", requireAuth, requireRole("leader"), async (req, res) => {
  const { id } = req.params;
...
organizationsRouter.delete("/:id", requireAuth, requireRole("leader"), async (req, res) => {
  const { id } = req.params;
  const existing = await prisma.organization.findUnique({ where: { id } });
  if (!existing) {
    res.status(404).json({ error: "Organization not found" });
