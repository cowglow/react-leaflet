...
export async function clickMap(page: Page, xRatio: number, yRatio: number): Promise<void> {
...
export async function addMemberViaMap(page: Page, options: AddMemberOptions): Promise<void> {
  const { firstName, lastName, xRatio = 0.5, yRatio = 0.5, street, number, zip, city } = options;
...
export async function openOrganizationsTree(page: Page): Promise<void> {
  if (await page.locator(".org-tree").isVisible().catch(() => false)) {
    return;
  }
...
export async function selectMemberInTree(page: Page, fullName: string): Promise<void> {
  ...
  const collapsedSummaries = page.locator(".org-tree details:not([open]) > summary");
  ...
  await expect(page.locator(".org-preview-card")).toContainText(fullName, { timeout: 5000 });
}
...
export async function moveSelectedMemberMarker(page: Page, dx: number, dy: number): Promise<void> {
  ...
  await page.locator('.title-bar:has-text("Map")').click({ position: { x: 5, y: 5 } });
  ...
  for (let i = 0; i < count; i++) {
    ...
    const distance = Math.hypot(
      ...
      markerBox.y + markerBox.height / 2 - centerY,
    );
